...
def prepare_data(data_dir, batch_size):
    ...
    return (train_clf_dl, train_O1_AE_dl, train_O2_AE_dl, train_O3_AE_dl), (
        test_clf_ds, test_clf_ds, test_O1_ds, test_O2_ds, test_O3_ds), label_weight, ord_enc.categories_[0]


def evaluate(model, testdata, idx2class, result_dir):
    model.eval()
    with torch.no_grad():
        yb = testdata[:][-1].cuda(0)
        preds, f_connect, f_out, tcp_confidence = model(testdata[:][0].cuda(0), testdata[:][1].cuda(0),
                                                        testdata[:][2].cuda(0))
...
def train_test(data_dir, result_dir, batch_size):
    ...
    test_clf_ds, test_clf_ds, test_O1_ds, test_O2_ds, test_O3_ds = dataset
