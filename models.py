...
class Subtyping_model(nn.Module):
    def __init__(self, O1_encoder, O2_encoder,O3_encoder, subtypes):
        ...
        self.ge_repr = nn.Sequential(*list(O1_encoder.children())[1:])
        self.cna_repr = nn.Sequential(*list(O2_encoder.children())[1:])
        self.d3_repr = nn.Sequential(*list(O3_encoder.children())[1:])
    ...
    def forward(self, x1, x2, x3):
        O1_ft = self.ge_repr(x1)
        O2_ft = self.cna_repr(x2)
        O3_ft = self.d3_repr(x3)
